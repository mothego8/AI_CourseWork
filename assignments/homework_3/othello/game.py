...
def whoIsFirst(s):
    ...
    "Please input 'b' if you want to be black and anything else if you want to be white: ").upper()
    # assigns color based on users imput. Black is always first
    if color == 'B':
        print("\n You are BLACK \n")
        HUMAN, COMPUTER = '○', '●'
        s[2] = HUMAN
...
def printState(s):
    ...
    # every time you print state you check whether the game is over
    # if theres no more moves for a player the game is finsished 
    # and the person with most coins wins
    if isFinished(s):
        s[1] = whoWin(s)
        if s[1] == VIC:
